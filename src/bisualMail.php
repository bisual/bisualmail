...
class bisualMail
{
    ...
    public static function generateMailable($request = null)
    {
        ...
        if (! self::getMailable('name', $name)->isEmpty()) {
            return response()->json([
                'status' => 'error',
                'message' => 'Ya existe un Mailable con ese nombre.',
            ]);
        }
        ...
        if (strtolower($name) === 'mailable') {
            return response()->json([
                'status' => 'error',
                'message' => 'No puedes usar el nombre mailable',
            ]);
        }
        ...
        $jobCode = Artisan::call('make:job', ['name' => $jobName]);
        $notifyCode = Artisan::call('make:notification', ['name' => $notifyName]);
        $mailCode = Artisan::call('make:mail', $params->all());
        
        if ($mailCode > -1 and $jobCode > -1 and $notifyCode > -1) {
            
            self::replaceMailFile($name, $notifyName);
            self::replaceNotifyFile($name, $notifyName);
            self::replaceJobFile($jobName, $notifyName);
    ...
    public static function replaceJobFile($jobName, $notifyName)
    {
        ...
        $toReplace = [
            'use Illuminate\Queue\SerializesModels;\nuse App\User;\nuse App\Notifications\\'.$notifyName.';\n',
            'Queueable, SerializesModels;\n\n    protected $args;\n',
            'public function __construct($args)\n    {\n        $this->args = $args;\n    }',
            'public function handle()\n    {\n        $user = User::where("email", $this->args["email"])->first();\n        $user->notify(new '.$notifyName.'($this->args));\n    }\n\n    public function tags()\n    {\n        return ["'.$jobName.' " . $this->args["email"]];\n    }'
        ];
        $new = str_replace($replace, $toReplace, $content);
        $replaced = str_replace(array('\r', '\n', '\t', '\v'), array("\r", "\n", "\t", "\v"), $new);

        file_put_contents($jobFile, $replaced);
    }

    public static function replaceNotifyFile($name, $notifyName)
    {
        ...
        $toReplace = [
            ...
            'use Queueable;\n\n    protected $args;\n',
            'public function __construct($args)\n    {\n        $this->args = $args;\n    }',
            'toMail($notifiable)\n    {\n        return (new '.$name.'($this->args))\n                    ->to($notifiable->email, $notifiable->name)\n                    ->subject(\''.$name.'\')\n                    ->from(\'contacto@mogaformacion.es\', "MOGA Formación");\n    }',
        ];
        $new = str_replace($replace, $toReplace, $content);
        $replaced = str_replace(array('\r', '\n', '\t', '\v'), array("\r", "\n", "\t", "\v"), $new);

        file_put_contents($notifyFile, $replaced);
    }

    public static function replaceMailFile($name, $notifyName)
    {
        $mailFile = app_path('Mail/'.$name.'.php');
