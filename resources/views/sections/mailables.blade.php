...
<div class="col-lg-10 col-md-12">
                ...
                <div class="card my-4">
                    <div class="card-header d-flex align-items-center justify-content-between"><h5>Acciones</h5>

                        @if (!$mailables->isEmpty() and auth()->user()->hasRole('super-admin'))
                        <a class="btn btn-primary" href="{{ route('backetfy.mails.createMailable') }}" data-toggle="modal" data-target="#newMailableModal">Añadir acción</a>
                        @endif
                        <!-- Modal -->
                    </div>
                    ...
                    @component('bisualmail::layout.emptydata')
                        <span class="mt-4">Ahora mismo no hay correos creados</span>
                    ...
                    <table id="mailables_list" class="table table-responsive table-hover table-sm mb-0 penultimate-column-right">
                        <thead>
                            <tr>
                                <th scope="col">{{ __('backetfy.name') }}</th>
                                @if (auth()->user()->hasRole('super-admin'))
                        ...
                        @foreach($mailables->all() as $mailable)
                            <tr id="mailable_item_{{ $mailable['name'] }}">
                                ...
                                @php
                                    ...
                                    if ($currentTemplate == 'view.name') $currentTemplate = 'No tiene plantilla asignada';
                                @endphp
                                <td class="table-fit"><span>{{ ucwords(preg_replace('/(?<!\ )[A-Z]/', ' $0',$currentTemplate)) }}</span></td>
                                
                                <td class="table-fit"><span>{{ (\Carbon\Carbon::createFromTimeStamp($mailable['modified']))->diffForHumans() }}</span></td>

                                <td class="table-fit">
                                    <a href="{{ route('backetfy.mails.viewMailable', ['name' => $mailable['name']]) }}" class="table-action mr-3"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 16"><path d="M16.56 13.66a8 8 0 0 1-11.32 0L.3 8.7a1 1 0 0 1 0-1.42l4.95-4.95a8 8 0 0 1 11.32 0l4.95 4.95a1 1 0 0 1 0 1.42l-4.95 4.95-.01.01zm-9.9-1.42a6 6 0 0 0 8.48 0L19.38 8l-4.24-4.24a6 6 0 0 0-8.48 0L2.4 8l4.25 4.24h.01zM10.9 12a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm0-2a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"></path></svg>
                                    </a>
  ...
  <form id="create_mailable" action="{{ route('backetfy.mails.generateMailable') }}" method="POST">
    @csrf
...
<script type="text/javascript">
    ...
    notie.confirm({
        ...
        text: '¿Seguro que quieres borrar esta acción?<br>Eliminar acción <b>'+ mailableName +'</b>',
        submitText: 'Eliminar',
        cancelText: 'Cancelar',

    submitCallback: function () {

    axios.post('{{ route('backetfy.mails.deleteMailable') }}', {
    ...
    .then(function (response) {
        ...
        if (response.data.status == 'ok'){
            ...
            notie.alert({ type: 1, text: 'Acción eliminada', time: 2 });
            ...
            notie.alert({ type: 'error', text: 'Acción no eliminada', time: 2 })
        }
    })
