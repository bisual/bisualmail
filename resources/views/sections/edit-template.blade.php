                      ...
                      <div class="card">
                        ...
                        <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
                          <div class="card-body">
                            ...
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Template View:</b> {{ 'bisualmail::templates.'.$template['slug'] }}</p>
							@endif

                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Tipo:</b> {{ ucfirst($template['template_type']) }}</p>
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Estilo:</b> {{ ucfirst($template['template_view_name']) }}</p>
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Estructura:</b> {{ ucfirst($template['template_skeleton']) }}</p>
                            <p class="text-primary edit-template" style="cursor:pointer;"><i class="fas fa-trash"></i> Editar detalles</p>
                            <p class="text-danger delete-template" style="cursor:pointer;"><i class="fas fa-trash "></i> Eliminar plantilla</p>
                        </div>
                        </div>
                      </div>
                    </div>
                ...
                <div class="col-lg-3 d-none d-lg-block">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Estructura:</b> {{ ucfirst($template['template_skeleton']) }}</p>
                            <p class="text-primary edit-template" style="cursor:pointer;"><i class="fas fa-trash"></i> Editar detalles</p>
                            <span class="text-danger delete-template" style="cursor:pointer;"><i class="fas fa-trash "></i> Eliminar plantilla</p>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function(){
	 ...
	 $('.edit-template').click(function(){
		...
		notie.input({
                  ...
                  submitCallback: function (templatedescription) {
	                  axios.post('{{ route('backetfy.mails.updateTemplate') }}', {
		...
		notie.confirm({
		    ...
		    .then(function (response) {
		    	...
		    	if (response.data.status == 'ok'){
				    ...
				    setTimeout(function(){
                                window.location.replace('{{ route('backetfy.mails.templateList') }}');
                    }, 3000);
	...
	var simplemde = new SimpleMDE(
		...
		previewRender: function(plainText, preview) {
			...
			$.ajax({
				  method: "POST",
				  url: "{{ route('backetfy.mails.previewTemplateMarkdownView') }}",
	...
	$('.save-template').click(function(){
		...
		notie.confirm({
	    	...
	    	submitCallback: function () {
	    		...
	    		axios.post('{{ route('backetfy.mails.parseTemplate') }}', { markdown: simplemde.codemirror.getValue(), viewpath: "{{ $template['slug'] }}", template: true })
