            ...
            <div class="row my-4">
                    ...
                    <div id="accordion">
                      <div class="card">
                        ...
                        <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
                          <div class="card-body">
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Acción:</b> <a href="{{ route('backetfy.mails.viewMailable', ['name' => $name]) }}">{{ $name }}</a></p>
                ...
                <div class="col-lg-9 col-md-12">
                    <div class="card mb-2">
                        <div class="card-header p-3" style="border-bottom:1px solid #e7e7e7e6;">
                            ...
                            <button type="button" class="btn btn-light float-right mr-2 save-draft disabled">Guardar borrador</button> <br>
                        </div>
                    </div>
                    ...
                    <ul class="nav nav-pills" id="pills-tab" role="tablist">
                      ...
                      <li class="nav-item">
                        <a class="nav-link {{ !is_null($templateData['text_template']) ? '' : 'disabled' }}" id="pills-profile-tab" data-toggle="pill" href="#pills-profile" role="tab" aria-controls="pills-profile" aria-selected="false">Texto plano</a>
                      </li>
                    </ul>
                            ...
                            @foreach($templateData['view_data'] as $param)
                                ...
                                @if (isset($param['data']['type']) && $param['data']['type'] === 'model')
                                    <div class="btn-group dropright">
                                      <button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" bisualmail-data-toggle="tooltip" data-placement="top" title="Elequent Model">
                                ...
                                @elseif(isset($param['data']['type']) && $param['data']['type'] === 'elequent-collection')
                                    ...
                                    <button type="button" class="btn btn-info btn-sm view_data_param" bisualmail-data-toggle="tooltip" data-placement="top" title="Elequent Collection" param-key="{{ $param['key'] }}">
                                    <i class="fa fa-table mr-1" aria-hidden="true"></i>{{ $param['key'] }}
                                    </button>

                                @elseif(isset($param['data']['type']) && $param['data']['type'] === 'collection')

                                    <button type="button" class="btn btn-success btn-sm view_data_param" bisualmail-data-toggle="tooltip" data-placement="top" title="Collection" param-key="{{ $param['key'] }}">
                                    <i class="fa fa-table mr-1" aria-hidden="true"></i>{{ $param['key'] }}
                                    </button>

                                @else
                                    <button type="button" class="btn btn-secondary btn-sm view_data_param" bisualmail-data-toggle="tooltip" data-placement="top" title="Simple Variable" param-key="{{ $param['key'] }}">
                ...
                <div class="col-lg-3 d-none d-lg-block">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Mailable:</b> <a href="{{ route('backetfy.mails.viewMailable', ['name' => $name]) }}">{{ $name }}</a></p>
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">Namespace:</b> {{ $templateData['namespace'] }}</p>
                            <p style="font-size: .9em;"><b class="font-weight-sixhundred">{{ $templateData['is_markdown'] ? 'Markdown' : 'View' }}:</b> {{ $templateData['template_name'] }}</p>
...
@if ( $templateData['is_markdown'] )
    ...
    var simplemde = new SimpleMDE(
        ...
        previewRender: function(plainText, preview) {
            $.ajax({
                  method: "POST",
                  url: "{{ route('backetfy.mails.previewMarkdownView') }}",
...
$('.view_data_param').click(function(){
    let link = $('.tox-pop.tox-pop--bottom');
    console.log();
    ...
    if (link.length) {
        link.find('input').val('');
        link.find('input').val(output);
        $(tinymce.activeEditor.selection.getNode()).attr('href', output);
        $(tinymce.activeEditor.selection.getNode()).attr('data-mce-href', output);
    } else {
